...
zone_max = {"DK1-onshore":3600, "DK2-onshore":700, "DK1-offshore":1260, "DK2-offshore":1000}
...
def load_observations(path, sep = ";", decimal = ","):
    ...
    data["offshore"] = data['OffshoreWindLt100MW_MWh'] + data['OffshoreWindGe100MW_MWh']
    data["onshore"] = data['OnshoreWindLt50kW_MWh'] + data['OnshoreWindGe50kW_MWh']

    # make columns
    data = data.pivot(columns="PriceArea", index = "Time", values = ["onshore", "offshore"])
...
class NABQRDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return torch.index_select(self.X, 0, idx - self.timesteps + self.start), self.Y[idx]

def train_model(X, Y, config, loss):

    data = NABQRDataset(X, Y)
    train, val = [DataLoader(x, batch_size=batch_size) for x in random_split(data, [train_size,1-train_size])]

    model = keras.Sequential.from_config(config)
    model.compile(loss=loss, optimizer=keras.optimizers.Adam())
    val_stop = keras.callbacks.EarlyStopping(monitor="val_loss", patience=10, min_delta=0.1, restore_best_weights=True)

    hist = model.fit(train, validation_data=val, epochs=epochs, callbacks=[val_stop])

    return model, hist

def apply_model(model, ensembles, observations, zone):

    data = DataLoader(NABQRDataset(ensembles.values, observations.values), batch_size=batch_size)

    quants = np.cumulative_sum(model.predict(data), axis = 1, include_initial=True)*zone_max[zone]
...
def calc_scores(actuals, simulations, window_size = 24):
    ...
    simulations = simulations[:n*window_size, :].reshape(-1, window_size, k).transpose(0,-1,-2)

    # mean score per "window" (i,j).
    # here ignoreing diagonal that is always 0
    vs = sr.vs_ensemble(actuals, simulations, p = 0.5) / (window_size*(window_size-1))
    vs = vs.mean()
...
#%% load data
observations = load_observations(PATH / "Data" / "Raw Data" / "ProductionConsumptionSettlement.csv" )
ensembles = load_ensembles(PATH / "Data" / "Raw Data" / "Ensembles")
observations = observations.loc[ensembles.index]

pd_index = pd.MultiIndex.from_product([zones, models_configs.keys(), observations.index.unique(1) ]).sortlevel()[0]
...
if TRAINNN:
    ...
    history = pd.DataFrame(
        index = pd.MultiIndex.from_product([zones, models_configs.keys(), np.arange(epochs+1)]).sortlevel()[0],
    ...
    for zone in zones:
        ...
        for name, config in models_configs.items(): 
            ...
            history.loc[zone, name, hist.epoch] = np.array(list(hist.history.values())).T
...
marginal_quantiles = pd.DataFrame(index = pd_index, columns = ["0.00"] + quantiles_str, dtype = np.float64)

for zone in zones:
    for name, model in models[zone].items():
        marginal_quantiles.loc[zone,name,:] = apply_model(model, ensembles.loc[zone], observations.loc[zone], zone)
...
for zone in zones:
    for name in models_configs.keys():
        tmp = transform2normal(observations[zone].values, marginal_quantiles.loc[zone,name].values)
...
for zone in zones:
    for name in models_configs.keys():
        ...
        if TRAINAR:
            ...
            params = np.load(p)
        model = sm.tsa.SARIMAX(np.zeros(30), order = order, seasonal_order = seasonal_order)
...
if TRAINAR:
    parameters = pd.concat([pd.concat(parameters[zone].values(), keys = parameters[zone].keys()) for zone in zones], keys = zones)
...
for zone in zones:
    for name in models_configs.keys():
        print(f"{zone}-{name}")
        simulations.loc[zone, name] = transform2orig(simulations_normal.loc[zone,name], marginal_quantiles.loc[zone,name].values)

simulations.to_pickle(PATH / "Data" / f"simulation.pkl" )

#%% scores

model_names = ["Ensembles"] + [k + " - Marginal" for k in models_configs.keys()] + list(models_configs.keys()) 

scores = pd.DataFrame(index = pd.MultiIndex.from_product([zones, model_names]), columns = ["MAE", "RMSE", "CRPS", "VarS"], dtype = np.float64)

for zone in zones:
    print(zone)
    for name in models_configs.keys():
        tmp = calc_scores(observations.loc[zone,test_idx].values, simulations.loc[zone, name, test_idx].values)
...
for zone in zones:
    print(zone)
    for name in models_configs.keys():
        vals = transform2orig(sim, marginal_quantiles.loc[zone,name, test_idx].values)
        tmp = calc_scores(observations.loc[zone,test_idx].values, vals)
        scores.loc[zone, name + " - Marginal", :] = tmp 

# original ensembles
for zone in zones:
    tmp = calc_scores(observations.loc[zone,test_idx].values, ensembles.loc[zone,test_idx,:].values)
